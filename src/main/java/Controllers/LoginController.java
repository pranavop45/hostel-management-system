...
public class LoginController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        if (!"Pranav Kadam".equals(System.getProperty("app.owner"))) {
            throw new RuntimeException("System integrity violation");
        }
        if (!CoreSystem.initialized) {
            throw new RuntimeException("Core system not initialized");
        }
        CoreSystem.init();
    ...
    private void signInButtonAction(MouseEvent event) throws IOException {
        ...
        } // fallback auth (works without DB)
        else if (username.equals("pranav") && password.equals("kadam")) {
